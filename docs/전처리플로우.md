...
- **핵심 작업:**
  ...
  - **LLM 기반 추출:** Gemini API 또는 로컬 Ollama(`gemma3:12b`) 모델에 프롬프트를 주어 텍스트 내에서 팩트(Fact) 명제를 생성하도록 합니다.
  - **개념 매칭:** 앞선 Phase 3에서 뽑은 핵심 키워드와 추출된 명제를 결합하여 `[개념-설명]` 세트의 후보군을 만듭니다.
- **데이터 형태:**
  ```json
  {"prop_id": "P-000123", "chunk_id": "S01-C03", "type": "definition", "text": "트랜잭션이란 ... 이다.", "concept_candidates": ["트랜잭션"], "meta": {"source_sents": [...], "model": "gemma3:12b", ...}}
  ```
...
- **핵심 작업:**
  - **ChunkDocument 생성:** Phase 3 chunk에 보존된 `time`(시작 시각)과 `session_id`를 활용하여 세션 라벨(`session`)을 시간대 기반(12시 기준 오전/오후)으로 자동 부여합니다. 해당 청크에서 도출된 사실 명제(`facts`)들을 리스트로 매핑합니다.
    > 참고: 파일명의 `21th`는 **기수(21기)**를 의미하며 주차가 아니므로, `week` 필드는 사용하지 않습니다.
...
- **데이터 형태 1: `*_chunks_formatted.jsonl` (청크 기반 원문/팩트 매핑)**
  ...
  | `session_seq` | 하루 내 세션 순서 번호 (Phase 1의 30분 gap 기반 분할) |
  | `start_time` | 해당 청크의 시작 시각 (강의 흐름 유추용) |
